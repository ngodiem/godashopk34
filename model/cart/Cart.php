...
class Cart
{
	...
	function addProduct($product_id, $qty) {
		...
		$item = array(
			...
			"url" => $router->generate("product-detail", ["slugName" => slugify($product->getName()), "id"=>$product->getId()])
	...
	protected function addItem($item) {
		...
		if (!array_key_exists($product_id, $this->items)) {
			$this->items[$product_id] = array(
				...
				"url" => $router->generate("product-detail", ["slugName" => slugify($name), "id"=>$product_id])
			);
